...
export default function GroupChatScreen() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'groupMessages'), where('groupId', '==', id));
      ...
      <KeyboardAvoidingView
        ...
        <View style={styles.header}>
          ...
          <View style={styles.groupSelector}>
            <Text style={styles.groupName}>{currentGroup?.name || '그룹'}</Text>
          </View>

          <TouchableOpacity
            style={styles.settingsButton}
            onPress={() => router.push({pathname: '/group-settings', params: {id}})}>
            <Ionicons name="settings-outline" size={24} color={BG} />
          </TouchableOpacity>
        </View>
        ...
        {/* 안내 메시지 */}
        <Text style={styles.notice}>{currentGroup?.name} 그룹에 입장하셨습니다.</Text>
